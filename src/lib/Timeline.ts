...
// API modules
import {Manipulations, Stimuli} from "./API";
...
// Import jQuery for exporting to Gorilla only
import $ from 'jquery';
...
window.onload = () => {
  // Get the images from using ImageCollection
  const _imageLoader = new Stimuli();
  const imageCollection = _imageLoader.get();

  if (Configuration.target === 'gorilla') {
    // Configure the manipulations in the configuration file as required
    new Manipulations(Configuration.trials, ['numTutorialTrials', 'numPracticeTrials', 'numCalibrationOneTrials', 'numMainTrials']);
  }
  ...
  if (keyLayout.name === 'spectrometer') {
    instructionContinueText =
      `<div id="instructions-navigation">` +
        `<br>` +
        `<hr>` +
        `<img src="${imageCollection['ControlsNavigationSpectrometer.png']}" ` +
    ...
    instructionContinueText =
      `<div id="instructions-navigation">` +
        `<br>` +
        `<hr>` +
        `<img src="${imageCollection['ControlsNavigationDesktop.png']}" ` +
          `style="${Configuration.style.controls}">` +
      `</div>`;
  }
  ...
  if (keyLayout.name === 'spectrometer') {
    leftControlImage = `<img src="${imageCollection['2.png']}" ` +
      `style="${Configuration.style.keyboard}"> `;
    rightControlImage = `<img src="${imageCollection['3.png']}" ` +
      `style="${Configuration.style.keyboard}"> `;
    submitControlImage = `<img src="${imageCollection['4.png']}" ` +
      `style="${Configuration.style.keyboard}"> `;
  } else {
    leftControlImage = `<img src="${imageCollection['F.png']}" ` +
      `style="${Configuration.style.keyboard}"> `;
    rightControlImage = `<img src="${imageCollection['J.png']}" ` +
      `style="${Configuration.style.keyboard}"> `;
    submitControlImage = `<img src="${imageCollection['K.png']}" ` +
      `style="${Configuration.style.keyboard}"> `;
  }
  ...
  if (keyLayout.name === 'spectrometer') {
    controlInstructions =
      `<div>` +
        `<img src="${imageCollection['ControlsQuestionSpectrometer.png']}" ` +
          `style="${Configuration.style.controls}">` +
      `</div>`;
  } else {
    controlInstructions =
      `<div>` +
        `<img src="${imageCollection['ControlsQuestionDesktop.png']}" ` +
          `style="${Configuration.style.controls}">` +
      `</div>`;
  }
  ...
  const description = [
    ...
    `<p>An example illustrating the appearance of these dots is ` +
      `shown below:</p>` +
    `<img src="${imageCollection['InstructionsMovingDots.gif']}" ` +
    ...
    `<img src="${imageCollection['InstructionsReference.png']}" ` +
        `style="${Configuration.style.image}"/>` +
    ...
    `<img src="${imageCollection['InstructionsConfidence.png']}" ` +
        `style="${Configuration.style.image}"/>` +
    `<p>Press ${leftControlImage} ` +
  ...
  if (keyLayout.name !== 'spectrometer') {
    timeline.push({
      ...
      feedback_correct: 'Correct! You will have to guess the ' +
        'direction of the moving dots.',
      feedback_incorrect: 'Incorrect. You will have to guess the ' +
        'direction of the moving dots.',
  ...
  for (let t = 0; t < Configuration.trials.numTutorialTrials; t++) {
    const trialName = 'tutorial';
    const d = parseFloat(((Math.random() *
      (tutorialDuration[1] - tutorialDuration[0]) +
      tutorialDuration[0]) * 1000).toFixed(2));
    const k = Math.random() > 0.5 ?
      tutorialCoherence[0] : tutorialCoherence[1];
  ...
  const practice = [
    `<h1>${Configuration.name} game</h1>` +
    `<h2>Practice Games</h2>` +
    `<p>You will now play another ${Configuration.trials.numPracticeTrials} ` +
      `practice games. ` +
  ...
  if (keyLayout.name !== 'spectrometer') {
    timeline.push({
      type: 'attention-check',
      question: 'How will you know if you have correctly guessed the ' +
      ...
      button_key: keyLayout.submit,
      button_text: 'Submit Answer',
      feedback_correct: 'Correct! The fixation cross in the circle ' +
        'will go green.',
      feedback_incorrect: 'Incorrect. The fixation cross in the ' +
        'circle will go green',
      instructions: controlInstructions,
    });
  }

  for (let t = 0; t < Configuration.trials.numPracticeTrials; t++) {
    const trialName = 'practice';
    let r = Math.random() > 0.5 ? 0 : Math.PI;
    r = parseFloat(r.toFixed(3));
    const k = Math.random() > 0.5 ?
      practiceCoherence[0] : practiceCoherence[1];
  ...
  if (Configuration.testing === true) {
    ...
    timeline.push({
      ...
      preamble: `Please answer the following questions relating to the ` +
          `instructions you just read before continuing ` +
          `to the main games.`,
      questions: [
        {prompt: `The instructions made sense to me.`,
          name: 'Question 1', labels: likertScale,
          required: true},
        {prompt: `The instructions helped me play the games.`,
          name: 'Question 2', labels: likertScale,
          required: true},
        {prompt: `I did not encounter anything unexpected while playing the ` +
          `games.`, name: 'Question 3', labels: likertScale,
        required: true},
      ],
  ...
  const main = [
    ...
    `${Configuration.trials.numCalibrationOneTrials +
        Configuration.trials.numMainTrials} ` +
    `games.</p>` +
  ...
  if (keyLayout.name !== 'spectrometer') {
    timeline.push({
      ...
      button_key: keyLayout.submit,
      button_text: 'Submit Answer',
      feedback_correct: 'Correct! You should focus on the fixation ' +
        'cross in the circle.',
      feedback_incorrect: 'Incorrect. You should focus on the ' +
        'fixation cross in the circle',
      instructions: controlInstructions,
    });
  }
  ...
  for (let t = 0; t < Configuration.trials.numCalibrationOneTrials; t++) {
  ...
  // -------------------- Main games --------------------
  for (let t = 0; t < Configuration.trials.numMainTrials; t++) {
  ...
  function calculateDuration() {
    ...
    let total = Configuration.trials.numTutorialTrials +
                Configuration.trials.numPracticeTrials +
                Configuration.trials.numCalibrationOneTrials +
                Configuration.trials.numMainTrials;
  ...
  if (Configuration.target === 'gorilla') {
    ...
    const _jsPsych: any = window['jsPsych'];
    ...
    if (_gorilla && _jsPsych) {
      ...
      _gorilla.ready(function() {
        _jsPsych.init({
          ...
          preload_images: Object.values(Configuration.images.gorilla),
        });
      });
    } else {
    ...
    if (_jsPsych) {
      ...
      _jsPsych.init({
        timeline: timeline,
        on_finish: function() {
          _jsPsych.data.get().localSave(`csv`, `rdk_${Date.now()}.csv`);
        },
        show_progress_bar: true,
        show_preload_progress_bar: true,
        preload_images: Object.values(Configuration.images.desktop),
      });
    } else {
